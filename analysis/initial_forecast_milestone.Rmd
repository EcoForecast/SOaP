...
Let's take a look at our driver ensembles for our calibration period:
```{r}
...
Load in our calibration model data:
```{r}
...
plot.run <- function(){
  sel = seq(1,10)
  plot(time,time,type='n',ylim=c(4,31), ylab="N")
...
Deterministic forecast (every input is a mean, rather than full distribution):
```{r}
ci <- apply(as.matrix(out$predict),2,quantile,c(0.025,0.5,0.975))
...
Now for driver uncertainty (precipitation and temperature, which came from the same 10 ensembles):
```{r}
## sample driver rows
...
We don't have a random site effect because our model isn't hierarchical. We're just looking at one NEON site (STER) right now. So let's go ahead and partition the variance!
```{r}
...
We have almost no driver or parameter uncertainty (driver uncertainty is there, but for some reason being masked by process error. Parameter uncertainty looks like it was too small to plot.) 